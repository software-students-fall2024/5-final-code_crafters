...
def test_create_user(test_client):
    ...
    response = test_client.post(
        "/users/create", json={"username": "testuser", "password": "testpass"}
    )
...
def test_authenticate_user(test_client):
    ...
    response = test_client.post(
        "/users/auth", json={"username": "testuser", "password": "testpass"}
    )
    assert response.status_code == 200
    data = response.get_json()
...
def test_add_todo(test_client):
    ...
    response = test_client.post(
        "/todo/add",
        json={"user_id": user_id, "exercise_item": exercise_item, "date": date_str},
    )
    assert response.status_code == 200
    data = response.get_json()
    assert data["success"]
...
def test_delete_todo(test_client):
    ...
    todo_entry = todo_collection.find_one(
        {"user_id": user_id, "date": datetime.strptime(today_date_str, "%Y-%m-%d")}
    )
    assert todo_entry is not None
...
def test_get_todo_item(test_client):
    ...
    todo_collection.insert_one(
        {"user_id": user_id, "date": datetime.utcnow(), "todo": [todo_item]}
    )
...
def test_get_exercise(test_client):
    ...
    exercise_id = exercises_collection.insert_one(
        {"workout_name": "Burpee", "description": "A full-body exercise."}
    ).inserted_id
...
def test_add_search_history(test_client):
    ...
    response = test_client.post(
        "/search-history/add", json={"user_id": user_id, "content": content}
    )
    assert response.status_code == 200
    data = response.get_json()
    assert data["success"]
...
def test_add_transcription(test_client):
    ...
    response = test_client.post(
        "/transcriptions/add", json={"user_id": user_id, "content": content}
    )
    assert response.status_code == 200
    data = response.get_json()
    assert "id" in data

    transcription = edit_transcription_collection.find_one(
...
def test_update_user(test_client):
    ...
    user_id = users_collection.insert_one(
        {"username": "testuser", "password": "hashedpassword", "age": 25}
    ).inserted_id

    response = test_client.put(f"/users/update/{user_id}", json={"age": 26})
    assert response.status_code == 200
    data = response.get_json()
    assert data["success"]
...
def test_get_all_exercises(test_client):
    ...
    exercises_collection.insert_many(
        [{"workout_name": "Sit-up"}, {"workout_name": "Jumping Jack"}]
    )
...
def test_get_todo_by_date(test_client):
    ...
    response = test_client.get(
        f"/todo/get_by_date/{user_id}",
        query_string={"start_date": date_yesterday, "end_date": date_today},
    )
    assert response.status_code == 200
    data = response.get_json()
    assert len(data) == 2
...
def test_add_or_skip_todo(test_client):
    ...
    assert result.get("created", False) or result.get(
        "exists", False
...
def test_create_user_missing_fields(test_client):
    ...
    response = test_client.post("/users/create", json={"username": "testuser"})
...
def test_create_user_duplicate_username(test_client):
    ...
    users_collection.insert_one({"username": "testuser", "password": "hashedpassword"})
...
def test_authenticate_user_invalid_credentials(test_client):
    ...
    response = test_client.post(
        "/users/auth", json={"username": "nonexistent", "password": "wrongpass"}
    )
...
def test_get_todo_by_date_missing_params(test_client):
    ...
    user_id = str(ObjectId())
    response = test_client.get(
        f"/todo/get_by_date/{user_id}", query_string={"start_date": "2024-01-01"}
    )
    assert response.status_code == 400
    data = response.get_json()
...
def test_get_todo_by_date_invalid_date_format(test_client):
    ...
    response = test_client.get(
        f"/todo/get_by_date/{user_id}",
        query_string={"start_date": "invalid-date", "end_date": "2024-01-02"},
    )
...
def test_get_exercise_by_id_missing_params(test_client):
    ...
    response = test_client.get(
        ...
        query_string={"date": "2024-01-01", "exercise_todo_id": "some_id"},
    )
    assert response.status_code == 400
    data = response.get_json()
    assert "error" in data
...
def test_plan_save_missing_fields(test_client):
    ...
    response = test_client.post("/plan/save", json={"user_id": str(ObjectId())})
    assert response.status_code == 400
    data = response.get_json()
...
def test_plan_save_exception(test_client, monkeypatch):
    ...
    response = test_client.post(
        "/plan/save", json={"user_id": str(ObjectId()), "plan": {"workout": "Running"}}
    )
    assert response.status_code == 500
    data = response.get_json()
    assert data["success"] == False
...
def test_add_search_history_missing_fields(test_client):
    ...
    response = test_client.post(
        "/search-history/add", json={"user_id": str(ObjectId())}
    )
    assert response.status_code == 400
    data = response.get_json()
    assert "error" in data
...
def test_add_transcription_missing_fields(test_client):
    ...
    response = test_client.post(
        "/transcriptions/add", json={"user_id": str(ObjectId())}
    )
    assert response.status_code == 400
    data = response.get_json()
    assert "error" in data
...
def test_add_user_db_error(test_client, monkeypatch):
    ...
    response = test_client.post(
        "/users/create", json={"username": "erroruser", "password": "pass"}
    )
    assert response.status_code == 500
    data = response.get_json()
...
def test_users_update_db_exception(test_client, monkeypatch):
    ...
    user_id = ObjectId()
    response = test_client.put(f"/users/update/{user_id}", json={"age": 30})
    assert response.status_code == 500
    data = response.get_json()
...
def test_todo_add_no_exercise_item(test_client):
    ...
    response = test_client.post(
        "/todo/add", json={"user_id": user_id, "date": date_str}
    )
    assert response.status_code == 400
    data = response.get_json()
    assert "error" in data
...
def test_todo_delete_exercise_no_exercise_id(test_client):
    ...
    date_str = datetime.utcnow().strftime("%Y-%m-%d")
    response = test_client.post(
        "/todo/delete_exercise", json={"user_id": user_id, "date": date_str}
    )
    assert response.status_code == 400
    data = response.get_json()
