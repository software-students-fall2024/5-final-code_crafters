...
def add_or_skip_todo(user_id):
    ...
    try:
        ...
        existing_todo = todo_collection.find_one(
            {
                ...
                "date": {
                    ...
                    "$lt": datetime.strptime(today_date, "%Y-%m-%d")
                    + timedelta(days=1),
                },
            }
        )
        ...
        if existing_todo:
            print(
                f"DEBUG: To-Do entry already exists for user {user_id} on {today_date}."
            )
        ...
        todo_collection.insert_one(
            {"user_id": str(user_id), "date": target_date, "todo": []}
        )
...
def get_todo(user_id):
    ...
    try:
        ...
        todo_data = todo_collection.find_one(
            {"user_id": user_id, "date": {"$gte": today_start, "$lt": today_end}}
        )
        ...
        print(
            f"WARNING: No To-Do data found for user {user_id} on {today_start.date()}"
        )
...
def add_todo():
    ...
    if not user_id or not exercise_item or not date:
        print(
            f"ERROR: Missing required fields - user_id: {user_id}, exercise_item: {exercise_item}, date: {date}"
        )
    ...
    try:
        ...
        if existing_todo:
            print(
                f"DEBUG: Updating existing todo for user_id: {user_id}, date: {target_date}"
            )
            result = todo_collection.update_one(
                {"_id": existing_todo["_id"]}, {"$push": {"todo": exercise_item}}
            )
            success = result.modified_count > 0
            message = (
                "New todo item added to existing entry"
                if success
                else "Failed to add todo item"
            )
            print(
                f"DEBUG: Update result - success: {success}, modified_count: {result.modified_count}"
            )
        else:
            print(
                f"DEBUG: Creating new todo entry for user_id: {user_id}, date: {target_date}"
            )
            new_todo = {
                "user_id": user_id,
                "date": target_date,
            ...
            message = (
                "New todo entry created"
                if success
                else "Failed to create new todo entry"
            )
            print(
                f"DEBUG: Insert result - success: {success}, inserted_id: {result.inserted_id if success else None}"
            )
...
def get_exercise_by_id():
    ...
    if not user_id or not date or not exercise_todo_id:
        return (
            jsonify({"error": "user_id, date, and exercise_todo_id are required"}),
            400,
        )
...
def update_exercise():
    ...
    if not user_id or not date or not exercise_todo_id or not update_fields:
        return (
            jsonify(
                {
                    "error": "user_id, date, exercise_todo_id, and update_fields are required"
                }
            ),
            400,
        )
    ...
    try:
        ...
        result = todo_collection.update_one(
            ...
            {
                "$set": {
                    **{f"todo.$.{key}": value for key, value in update_fields.items()}
                }
            },
        )
        ...
        if result.modified_count > 0:
            return (
                jsonify({"success": True, "message": "Exercise updated successfully"}),
                200,
            )
...
def add_search_history():
    ...
    search_entry = {"user_id": user_id, "content": content, "time": datetime.utcnow()}
        ...
        for todo in todos:
            ...
            if "todo" in todo:
                for item in todo["todo"]:
                    if "time" in item:
                        item["time"] = datetime.fromisoformat(item["time"]).strftime(
                            "%Y-%m-%d"
                        )
...
def save_plan():
    ...
    try:
        ...
        if not user_id or not plan_data:
            return (
                jsonify(
                    {"success": False, "message": "user_id and plan data are required"}
                ),
                400,
            )
        ...
        plan_entry = {"user_id": user_id, "date": date, "plan": plan_data}
...
def delete_exercise_from_date():
    ...
    if not user_id or not date or not exercise_id:
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": "user_id, date, and exercise_id are required",
                }
            ),
            400,
        )
    ...
    try:
        ...
        if not user_todo:
            print(
                f"DEBUG: No todos found for User ID: {user_id} on Date: {target_date}"
            )
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "No todos found for the specified date",
                    }
                ),
                404,
            )
        ...
        if result.modified_count > 0:
            print(
                f"DEBUG: Successfully deleted exercise. User ID: {user_id}, Exercise ID: {exercise_id}"
            )
            return (
        ...
        print(
            f"DEBUG: Exercise ID: {exercise_id} not found in User ID: {user_id}'s todo list for Date: {target_date}"
        )
